            ...
            for (var m = 0, y = 0; m < r.length; y += r[m].length, ++m) {
              ...
              if (!(v instanceof a)) {
                ...
                if (!b && h && m != r.length - 1) {
                  ...
                  for (var w = b.index + (c ? b[1].length : 0), _ = b.index + b[0].length, P = m, A = y, j = r.length; j > P && _ > A; ++P) {
...
!function (a) {
  var e = a.util.clone(a.languages.javascript);a.languages.jsx = a.languages.extend('markup', e), a.languages.jsx.tag.pattern = /<\/?[\w\.:-]+\s*(?:\s+(?:[\w\.:-]+(?:=(?:("|')(\\?[\w\W])*?\1|[^\s'">=]+|(\{[\w\W]*?\})))?|\{\.{3}\w+\})\s*)*\/?>/i, a.languages.jsx.tag.inside['attr-value'].pattern = /=(?!\{)(?:('|")[\w\W]*?(\1)|[^\s>]+)/i, a.languages.insertBefore('inside', 'attr-name', {
    spread: {
      pattern: /\{\.{3}\w+\}/,
